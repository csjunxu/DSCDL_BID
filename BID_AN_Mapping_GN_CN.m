...
model.net = getfield(load('./GatingNetwork/GatingNet_knnNI2BS500.mat'),'net');
...
for i = 1:im_num
    ...
    for nh = listh
        for nw = listw
            ...
            while Continue
                fprintf('Iter: %d \n', nOuterLoop);
                IMout_part_y = Mapping_BID(IMin_part_y,model,Dict,par,param);
    ...
    %% output
    imwrite(IMout, ['./Mapping_BID_AN_GN_CN/Mapping_BID_AN_GN_CN_' type '_' IMname '.png']);
    fprintf('The Loops of these parts are  %s. \n', num2str(nOuterLoop));
    Loops = nOuterLoop*par.nInnerLoop;
    save(['./Mapping_BID_AN_GN_CN/Mapping_BID_AN_GN_CN_' IMname '_Loops.mat'],'Loops');
end