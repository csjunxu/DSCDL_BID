...
for cc = 1:size(XN,3)
    [model, llh, cls_idx]  =  empggm(XC{cc}, cls_num,par.nlsp);
    for c = 1 : cls_num
        idx = find(cls_idx == c);
        %     if (length(idx) >  10000)
    ...
    % clear XN XC XN0 XC0;
    % save model
    GMM_model = ['Data/GMM_PG_Ch' num2str(cc) '_' num2str(par.nlsp) '_' num2str(patch_size) 'x' num2str(patch_size) '_' num2str(cls_num) '_' datestr(now, 30) '.mat'];
